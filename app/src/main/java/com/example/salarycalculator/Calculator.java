...
public class Calculator {
    ...
    private static double EducationFundToTax;
    private static double SocialSecurity;
    private static double HealthTax;
    private static double PensionFund;
    private static double NET;
    private static double TaxWithoutCreditsPointsReduces;
    public Calculator(int gross) {
        GROSS_SALARY = gross;
        ...
        EducationFundToTax = (EducationFundAll > EducationFundTaxFree) ? (EducationFundAll - EducationFundTaxFree) * 3 : 0;


        SocialSecurity = getSocialSecurity(GROSS_SALARY + EducationFundToTax, false);
        HealthTax = getHealthTax(GROSS_SALARY + EducationFundToTax, false);
        PensionFund = getPensionFund(BASE_FROM_SALARY);
        ...
        double EmployerLossOfAbilityToWorkSecretions = BASE_FROM_SALARY * LOSS_OF_ABILITY_TO_WORK_PERCENT;
         ...
         TaxWithoutCreditsPointsReduces = calculateIncomeTaxWithCreditsPoints(GROSS_SALARY + EXTRA_REDUCES + EducationFundToTax, 0);
...
//        System.out.println("- - - - - - - - - - - -");
         ...
         NET = GROSS_SALARY - NEW_TAX - SocialSecurity - HealthTax - PensionFund - EducationFundAll;
...
//        print("NET", NET);
    ...
    public static double getEducationFund() {
        return EducationFundToTax;
    }

    public static double getPensionFund(){
        return PensionFund;
    }
    public static double getHealthTax(){
        return HealthTax;
    }
    public static double getSocialSecurity(){
        return SocialSecurity;
    }
    public static double getIncomeTax(){
        return TaxWithoutCreditsPointsReduces;
    }
